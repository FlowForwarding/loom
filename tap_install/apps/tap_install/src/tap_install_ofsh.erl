...
-define(OFS_STATE, tap_install_ofs_state).
-record(?OFS_STATE, {
                    datapath_id,
                    controller,
                    aux_id = 0
...
% The callback functions in turn call tap_install_logic for processing.
...
init(Mode, IpAddr, DatapathId, _Features, Version, Connection, _Opts) ->
    % new main connection
    Pid = tap_install_logic:ofsh_init(Mode, IpAddr, DatapathId, Version, Connection),
    State = #?OFS_STATE{datapath_id = DatapathId, controller = Pid},
    {ok, State}.
...
connect(Mode, IpAddr, DatapathId, _Features, Version, Connection, AuxId, _Opts) ->
    ...
    ok = tap_install_logic:ofsh_connect(Mode, IpAddr, DatapathId, Version, Connection, AuxId),
...
disconnect(State) ->
    ...
    ok = tap_install_logic:ofsh_disconnect(AuxId, DatapathId).

-spec failover(ofs_state()) -> ok.
failover(State) ->
    % State of new active
    % TODO: not failover not implement in ofs_handler
    ok = tap_install_logic:ofsh_failover(),
    {ok, State}.
...
handle_error(Reason, State) ->
    ...
    ok = tap_install_logic:ofsh_handle_error(DatapathId, Reason).

-spec handle_message(ofp_message(), ofs_state()) -> ok.
handle_message(Msg, State) ->
    % received a message on the connection
    DatapathId = State#?OFS_STATE.datapath_id,
    ok = tap_install_logic:ofsh_handle_message(DatapathId, Msg).

-spec terminate(ofs_state()) -> ok.
terminate(State) ->
    % lost the main connection
    DatapathId = State#?OFS_STATE.datapath_id,
    Pid = State#?OFS_STATE.controller,
    ok = tap_install_logic:ofsh_terminate(DatapathId, Pid).
