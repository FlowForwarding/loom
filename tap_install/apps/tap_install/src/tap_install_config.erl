...
%% @doc tap module for parsing the tap_install.config file

-module(tap_install_config).
...
getenv(Key) ->
    case application:get_env(tap_install, Key) of
    ...
    case consult() of
        {ok, Config} ->
            case {proplists:get_value(Key, Config),
                                    application:get_env(tap_install, Key)} of
                {undefined, undefined} ->
                    {error, not_found};
