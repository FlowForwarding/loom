...
%%% @doc
%%% Tapestry callback handler for ofs_handler.
%%% @end
...
disconnect(State) ->
    ...
    ok = tap_loom:ofsh_disconnect(AuxId, DatapathId).
...
handle_message(Msg, State) ->
    ...
    DatapathId = State#?OFS_STATE.datapath_id,
    ok = tap_loom:ofsh_handle_message(DatapathId, Msg).

-spec terminate(ofs_state()) -> ok.
terminate(State) ->
    % lost the main connection
    DatapathId = State#?OFS_STATE.datapath_id,
    ok = tap_loom:ofsh_terminate(DatapathId).
